...
var _ = Describe("Set pthread scheduling type and priority", func() {
	...
	Context("when parsing the thread command for CPU ID", func() {
		DescribeTable("extracts the CPU ID", func(comm []byte, cpuID string, parseOK bool) {
			...
			Entry("Correctly extracts the CPU ID from a single digit vcpu pthread", []byte("CPU 3/KVM"), "3", true),
			Entry("Correctly extracts the CPU ID from a double digit vcpu pthread", []byte("CPU 10/KVM"), "10", true),
			Entry("Fails to parse the comm value", []byte("vCPU 1/KVM"), "", false),
			Entry("Fails to parse a negative vCPU value", []byte("CPU -1/KVM"), "", false))
	})
})

var _ = Describe("Parse the vCPU mask", func() {
	Context("when only a mask range is available (N-M)", func() {
